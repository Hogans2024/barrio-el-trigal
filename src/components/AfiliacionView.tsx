            ...
            <div className="space-y-5">
              ...
              {searchPerformed && searchFoundResult ? (
                ...
                (() => {
                  ...
                  return (
                    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="bg-[#0b101c] rounded-2xl sm:rounded-[32px] p-5 sm:p-8 flex flex-col gap-6 sm:gap-8 shadow-2xl border border-white/[0.03]">
                      {/* HEADER */}
                      <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
                        <div className="flex gap-4 sm:gap-6 items-center">
                          ...
                          <div className="space-y-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                              <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider">Padrón Activo</span>
                        ...
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                          onClick={() => { playTone(500, 80); setSearchFirstName(''); setSearchLastName(''); setSearchCI(''); setSearchPhone(''); setSearchPerformed(false); setSearchFoundResult(null); }}
                        ...
                        <motion.div variants={itemVariants} className="md:col-span-1 bg-white/[0.02] border border-white/5 rounded-3xl p-6 flex flex-col justify-between group hover:bg-white/[0.04] transition-colors relative overflow-hidden">
                          <div className="space-y-1 z-10">
                            ...
                            <div className="flex items-end gap-2">
                              <span className="text-5xl font-bold text-white tracking-tighter leading-none">{activity.pct === 100 ? '94' : Math.round(activity.pct)}</span>
                              <span className="text-emerald-400 font-mono font-bold text-sm mb-1">/100</span>
                            </div>
                          </div>
                          <div className="mt-8 z-10 space-y-3">
                            <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                              <motion.div initial={{ width: 0 }} animate={{ width: `${activity.pct === 100 ? 94 : Math.round(activity.pct)}%` }} transition={{ duration: 1.5, ease: "easeOut", delay: 0.5 }} className="h-full bg-emerald-400 rounded-full" />
                            </div>
                        ...
                        <div className="md:col-span-1 flex flex-col gap-6">
                          <motion.div variants={itemVariants} className="flex-1 bg-white/[0.02] border border-white/5 rounded-3xl p-6 flex flex-col justify-center group hover:bg-white/[0.04] transition-colors">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center flex-shrink-0"><Shield className="w-5 h-5 text-blue-400" /></div>
                              <div><span className="text-2xl font-bold text-white leading-none block mb-1">17</span><span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider font-bold">Activaciones de Alarma</span></div>
                            </div>
                          </motion.div>
                          <motion.div variants={itemVariants} className="flex-1 bg-white/[0.02] border border-white/5 rounded-3xl p-6 flex flex-col justify-center group hover:bg-white/[0.04] transition-colors">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-full bg-yellow-500/10 flex items-center justify-center flex-shrink-0"><Coins className="w-5 h-5 text-yellow-400" /></div>
                              <div><span className="text-2xl font-bold text-white leading-none block mb-1">Al Día</span><span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider font-bold">Estado Financiero</span></div>
                            </div>
                          </motion.div>
                        </div>
                          ...
                          <div className="space-y-4 flex-1 flex flex-col justify-center">
                            {[
                              ...
                              <div key={idx} className="flex justify-between items-center text-sm">
                                ...
                                {doc.done ? <CheckCircle className="w-4 h-4 text-emerald-400" /> : <X className="w-4 h-4 text-red-400" />}
                              </div>
                            ))}
                          </div>
                        </motion.div>
                          ...
                          {[90, 100, 95, 80, 100, 100].map((val, idx) => (
                            <div key={idx} className="flex-1 flex flex-col items-center gap-3 group/bar">
                              <div className="w-full h-24 sm:h-32 bg-black/20 rounded-t-lg relative flex items-end justify-center">
                                <motion.div initial={{ height: 0 }} animate={{ height: `${val}%` }} transition={{ duration: 1, ease: "easeOut", delay: 0.6 + (idx * 0.1) }} className="w-full mx-1 sm:mx-4 bg-emerald-500/50 rounded-t-md group-hover/bar:bg-emerald-400 transition-colors relative">
                                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-white opacity-0 group-hover/bar:opacity-100 transition-opacity">{val}%</div>
                                </motion.div>
                              </div>
                              <span className="text-[10px] font-mono text-gray-500 uppercase font-bold tracking-wider">{['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun'][idx]}</span>
                            </div>
                          ))}
                        </div>
                      </motion.div>
                ...
                <div className="bg-white/[0.02] border border-white/10 rounded-2xl sm:rounded-[24px] p-4 sm:p-6 space-y-5 animate-fade-in shadow-xl">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5 col-span-1 sm:col-span-2 md:col-span-1">
                      ...
                      <input type="text" placeholder="Ej. Daniel" value={searchFirstName}
                        onChange={(e) => { playTone(1200, 15); setSearchFirstName(e.target.value); setSearchPerformed(false); }}
                    ...
                    <div className="space-y-1.5 col-span-1 sm:col-span-2 md:col-span-1">
                      ...
                      <input type="text" placeholder="Ej. Mendez" value={searchLastName}
                        onChange={(e) => { playTone(1200, 15); setSearchLastName(e.target.value); setSearchPerformed(false); }}
                        className="w-full bg-black/40 border border-white/10 focus:border-[#FFD700] rounded-xl px-3 py-2 text-xs text-white focus:outline-none transition-all placeholder-gray-600 font-sans" />
                    </div>
                    <div className="space-y-1.5 col-span-1 sm:col-span-2 md:col-span-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider font-mono">Documento de Identidad C.I.</label>
                      <input type="text" placeholder="Ej. 12345678" value={searchCI}
                        onChange={(e) => { playTone(1200, 15); setSearchCI(e.target.value); setSearchPerformed(false); }}
                    ...
                    <div className="space-y-1.5 col-span-1 sm:col-span-2 md:col-span-1">
                      ...
                      <input type="text" placeholder="Ej. 12345678" value={searchPhone}
                        onChange={(e) => { playTone(1200, 15); setSearchPhone(e.target.value); setSearchPerformed(false); }}
                        className="w-full bg-black/40 border border-white/10 focus:border-[#FFD700] rounded-xl px-3 py-2 text-xs text-white focus:outline-none transition-all placeholder-gray-600 font-mono" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-2 pt-1">
                    {(searchFirstName || searchLastName || searchCI || searchPhone || searchPerformed) && (
                      <button onClick={() => { playTone(500, 80); setSearchFirstName(''); setSearchLastName(''); setSearchCI(''); setSearchPhone(''); setSearchPerformed(false); setSearchFoundResult(null); }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (!found && normalizeText(searchFirstName) === 'daniel' && normalizeText(searchLastName) === 'mendez' && normalizeText(searchCI) === '12345678') {
                          found = { nombre: 'Daniel Mendez', ci: '12345678', celular: searchPhone.trim() || '12345678', calle: 'Calle Las Acacias #44', estado: 'Activo' };
                    ...
                    <div key={modalRow.id} className="fixed left-0 right-0 z-50 flex justify-center px-4 animate-fade-in" style={{ bottom: `calc(${navHeight}px + env(safe-area-inset-bottom, 0px) + 121px)` }} onClick={e => e.stopPropagation()}>
                      ...
                      <div className="flex gap-2 px-3 py-2.5 bg-[#070707]">
                        <button
                          ...
                          className="flex-1 bg-white/10 text-white border-white/40 hover:bg-white/20 rounded-lg py-2 text-xs font-semibold border cursor-pointer transition"
                        >
                          Cerrar
                        </button>
                        <button
                          ...
                          className="flex-1 bg-[#FFD700]/10 text-[#FFD700] border-[#FFD700]/40 hover:bg-[#FFD700]/20 rounded-lg py-2 text-xs font-semibold border cursor-pointer transition active:scale-95"
                        >
