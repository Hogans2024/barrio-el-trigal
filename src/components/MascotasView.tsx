...
function buildPetSlides(pet: LostPet, petIdx: number): string[] {
  ...
  if (slides.length < slideCount) {
    const fallbacks = SLIDE_FALLBACKS.filter(fb => !slides.includes(fb));
    slides.push(...fallbacks.slice(0, slideCount - slides.length));
  }
  return slides;
}
...
const DEFAULT_IMAGES: Record<string, string> = {
  ...
  Tortuga: 'https://images.unsplash.com/photo-1570458436416-b8fcccfe883f?w=600&auto=format&fit=crop&q=80',
  Otras: 'https://images.unsplash.com/photo-1548767797-d8c844163c4c?w=600&auto=format&fit=crop&q=80',
};

export default function MascotasView({ mascotas, onShowNotification, highlightId, onClearHighlight, onRegisterBackHandler }: MascotasViewProps) {
    ...
    <div className="flex flex-col space-y-5 relative">
      ...
      <div className="-mt-[16px]">
        ...
        <style>{`
          ...
          .cat-icon-animate {
            animation: icon-slide 0.4s ease-out forwards;
          }
        `}</style>
      </div>
            ...
            <div className={`w-full pt-1.5 pb-1 flex items-center ${isMobile ? '' : 'justify-center'}`}>
              <div ref={buttonsRef} className={`flex items-center flex-nowrap ${isMobile ? 'w-full justify-between gap-0' : 'justify-center'}`} style={!isMobile ? { gap: 'clamp(4px, calc((100vw - 320px) / 12), 19px)' } : undefined}>
                ...
                <button
                  ...
                  className="relative inline-flex items-center space-x-1.5 px-2 py-1.5 rounded-lg text-[10px] font-semibold transition cursor-pointer border bg-sky-500/10 text-sky-400 border-sky-500/40 hover:bg-sky-500/20"
                >
            ...
            <div className="p-3 space-y-1">
              {categories.map((cat, idx) => {
                ...
                return (
                  ...
                  >
                    <span className="cat-icon-animate" style={{ animationDelay: `${idx * 60}ms` }}>
                      {categoryIcons[cat]}
                    </span>
                ...
                <div className="w-[45%] tall:w-[62%] p-2 tall:p-3.5 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-1.5 mb-1.5">
                      <h3 className="text-white text-sm font-bold leading-tight group-hover:text-[#FFD700] transition truncate"><span className={pet.status === 'adoption' ? 'text-blue-400' : pet.status === 'found' ? 'text-emerald-400' : 'text-yellow-400'}><span className="text-white">{idx + 1}.</span> {pet.type}</span> {pet.name}</h3>
                    </div>
              ...
              >
                ...
                <div className="p-4 flex flex-col space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-white text-base font-bold tracking-tight truncate"><span className={pet.status === 'adoption' ? 'text-blue-400' : pet.status === 'found' ? 'text-emerald-400' : 'text-yellow-400'}><span className="text-white">{idx + 1}.</span> {pet.type}</span> {pet.name}</h3>
                  </div>
            ...
            return (
              ...
              >
                ...
                <div className="p-4 space-y-3">
                  <div>
                    <h3 className="text-white text-base font-bold tracking-tight">Se busca a "<span className={pet.status === 'adoption' ? 'text-blue-400' : pet.status === 'found' ? 'text-emerald-400' : 'text-yellow-400'}><span className="text-white">{idx + 1}.</span> {pet.type}</span> {pet.name}"</h3>
          ...
          <div className="bg-[#080a0f] border border-white/10 rounded-2xl w-full max-w-md overflow-y-auto max-h-full animate-in fade-in zoom-in duration-200">
            {(() => {
              ...
              return (
                <>
                  <div
                    className={`relative bg-gray-950 overflow-hidden ${singleHorizontal ? 'h-36' : 'h-[264px]'}`}
                ...
                <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2 z-[90] pointer-events-auto">
                  {slides.map((src, idx) => (
                    <button
                      ...
                      className={`w-14 h-14 rounded-lg border-2 overflow-hidden cursor-pointer ${fullscreenImg === src ? 'border-[#FFD700] opacity-100 shadow-[0_0_10px_rgba(255,215,0,0.5)]' : 'border-transparent opacity-50 hover:opacity-100'}`}
                    >
      ...
      {showFormPicker && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#080a0f] border border-white/10 rounded-2xl w-full max-w-xs animate-in fade-in zoom-in duration-150">
            <div className="p-4 bg-[#FFD700]/10 text-[#FFD700] border-b border-[#FFD700]/20 flex justify-between items-center">
              <h4 className="font-extrabold text-sm tracking-tight flex items-center gap-1.5">
                <PawPrint className="h-4 w-4" />
                <span>Añadir Mascota</span>
              </h4>
              <button type="button" onClick={() => setShowFormPicker(false)} className="hover:opacity-75 text-gray-400">
                <X className="h-4 w-4 stroke-[2.5]" />
              </button>
            </div>
            <div className="p-5 space-y-3">
              <button
                onClick={() => { setFormMode('lost'); setShowFormPicker(false); setShowForm(true); }}
                className="w-full flex items-center justify-center gap-2 bg-yellow-500/10 text-yellow-400 border border-yellow-500/40 hover:bg-yellow-500/20 px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer"
              >
                <PawPrint className="h-4 w-4" />
                <span>Añadir Mascota Extraviada</span>
              </button>
              <button
                onClick={() => { setFormMode('found'); setShowFormPicker(false); setShowForm(true); }}
                className="w-full flex items-center justify-center gap-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/20 px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer"
              >
